...
// Helper function to get teacher's assigned class and subject IDs
const getTeacherAssignments = async (userId: string) => {
  try {
    ...
    if (!teacherSnapshot.empty) {
      ...
      // Get assigned classes (as references or strings)
      const classRefs = teacherData.classes || [];
      const classIds = classRefs.map((ref: any) => 
        typeof ref === 'string' ? ref.split('/').pop() : ref?.path?.split('/').pop()
      ).filter(Boolean);
      
      // Get assigned subjects (as references or strings)
      const subjectRefs = teacherData.subjects || [];
      const subjectIds = subjectRefs.map((ref: any) => 
        typeof ref === 'string' ? ref.split('/').pop() : ref?.path?.split('/').pop()
      ).filter(Boolean);
...
export default function DiaryManagementPage() {
  ...
  useEffect(() => {
    if (!authLoading) {
      if (schoolId && user?.uid) {
        // First fetch teacher's assigned classes and subjects
        getTeacherAssignments(user.uid)
          .then(({ classIds, subjectIds }) => {
            setTeacherClasses(classIds);
            setTeacherSubjects(subjectIds);
            
            if (classIds.length > 0) {
              fetchDiaryEntries(classIds);
            } else {
              // If teacher has no assigned classes, show empty state
              setDiaryEntries([]);
              setLoading(false);
              setError("No classes assigned to your account. Please contact your administrator.");
            }
          })
          .catch(error => {
            console.error("Error fetching teacher assignments:", error);
        ...
        setError(schoolId ? "User not authenticated." : "School ID not found. Please ensure you're logged in as a school administrator.");
        setLoading(false);
      }
    }
  }, [schoolId, authLoading, user?.uid]);

  const fetchDiaryEntries = async (classIds: string[]) => {
    ...
    try {
      // Create references to the teacher's classes for the query
      const classRefs = classIds.map(classId => doc(db, "classes", classId));
      
      // Batch queries to handle Firestore's 'in' limit of 10
      const batchSize = 10;
      const batches = [];
      
      for (let i = 0; i < classRefs.length; i += batchSize) {
        const batch = classRefs.slice(i, i + batchSize);
        batches.push(batch);
      }
      ...
      for (const batch of batches) {
        // Fetch homework entries for these classes
        ...
        // Fetch remark entries for these classes
        ...
        const [homeworkSnapshot, remarkSnapshot] = await Promise.all([
  ...
  const handleFormSuccess = () => {
    ...
    if (teacherClasses.length > 0) {
      fetchDiaryEntries(teacherClasses);
    }
  };
        ...
        <div className="max-w-7xl mx-auto">
              ...
              <div className="flex flex-wrap gap-4">
                ...
                {(filterType !== "all" || filterPriority !== "all" || searchTerm) && (
                  <button
                    ...
                    className="w-8 h-8 p-2 bg-gray-100 text-gray-500 hover:bg-gray-200 transition-colors duration-200 flex items-center justify-center rounded-full"
                  >
          ...
          {error && (
            <div className="bg-red-50 border border-red-200 text-red-700 px-6 py-4 rounded-2xl mb-6 shadow-sm">
              ...
              <button
                onClick={() => teacherClasses.length > 0 && fetchDiaryEntries(teacherClasses)}
                  ...
                  {filteredEntries.map((entry, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-0 h-0 border-4 border-transparent border-t-purple-500 border-b-purple-500 rounded-full" />
                        ...
                        <div className="p-6">
                          ...
                          <div className="flex gap-3 pt-4 border-t border-gray-100">
                            ...
                            >
                              <FontAwesomeIcon icon={faEdit} className="group-hover:rotate-12 transition-transform duration-300" />
                              Edit
                            </button>
                            <button
                            ...
                            >
                              <FontAwesomeIcon icon={faTrash} className="group-hover:scale-110 transition-transform duration-300" />
                              Delete
                            </button>
                          </div>
                        </div>
